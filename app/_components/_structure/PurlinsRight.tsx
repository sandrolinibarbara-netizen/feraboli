...
export default function PurlinsRight({material} : {material : THREE.Material}) {
    ...
    const PURLINRIGHT = () => {
        useLayoutEffect(() => {
            if(ref.current && halfPurlins && eavesHeight && roofIncline.percentage && width && length) {
                const mesh = new THREE.Object3D();

                for(let i = 0; i < halfPurlins; i++) {
                    const h = (((1 + 0.05) * i) - 0.1) * Math.sin(roofIncline.rad!);
                    const b = Math.sqrt(Math.pow(((1 + 0.05) * i) - 0.1, 2) - Math.pow(h, 2))
                    ...
                    mesh.position.set(i === 0 ? (width / 2) : (width / 2) - b, purlinHeight, -length / 2);
